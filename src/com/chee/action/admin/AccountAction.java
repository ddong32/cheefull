...
public class AccountAction extends BaseAction<Account, Integer> {
			...
			for (Account account : accountList) {
				...
				map.put("sflx",
						(String) StaticUtils.getSflxMap()
								.get(account.getSflx()));
				if (("1".equals(account.getSflx()))
						|| (account.getSflx() == "1")) {
					if ((account.getBusinessCustomer() != null)
							&& (account.getBusinessCustomer().getCustomer() != null)
							&& (account.getBusinessCustomer().getCustomer()
									.getPathName() != null)) {
						map.put("sfmc", account.getBusinessCustomer()
								.getCustomer().getPathName());
					} else {
						map.put("sfmc", "");
					}
				} else if ((account.getCooperator() != null)
						&& (account.getCooperator().getDwmc() != null)) {
		...
		if (this.ids != null) {
			try {
				this.accountList = this.accountService
						.findAccountUnique(Integer.valueOf(this.ids[0]
								.intValue()));
				...
				if ((this.account.getBusinessCustomer() != null)
						&& (this.account.getBusinessCustomer().getId() != null)) {
					map.put("bucId", this.account.getBusinessCustomer().getId());
				} else {
					map.put("bucId", "");
				}
				if ((this.account.getBank() != null)
						&& (this.account.getBank().getId() != null)) {
					map.put("bankId", this.account.getBank().getId());
				} else {
					map.put("bankId", "");
				}
				if ((this.account.getCooperator() != null)
						&& (this.account.getCooperator().getId() != null)) {
				...
				if (this.account.getSfsj() != null) {
					map.put("sfsj", DateUtil.formatDate(this.account.getSfsj(),
							"yyyy-MM-dd"));
				} else {
					map.put("sfsj", "");
				}
	...
	public String orderInput() {
		if ((this.orderId != null) && (!"".equals(this.orderId))) {
			if (this.ids != null) {
				this.account = ((Account) this.accountService.get(Integer
						.valueOf(this.ids[0].intValue())));
			}
		} else {
			...
			try {
				...
				for (Integer id : this.ids) {
					...
					if ("1".equals(persistent.getStat())) {
						...
						flag = true;
						stat = persistent.getOrderId() + ": 该收款状态为"
								+ persistent.getStat() + "不能招回操作！";
						break;
					}
				}
				if (flag) {
	...
	public String loadAccount() {
		...
		for (Account temp : list) {
			...
			map.put("sflx",
					(String) StaticUtils.getSflxMap().get(temp.getSflx()));
			if (("1".equals(temp.getSflx())) || (temp.getSflx() == "1")) {
				if ((temp.getBusinessCustomer() != null)
						&& (temp.getBusinessCustomer().getCustomer() != null)) {
					map.put("sfmc", temp.getBusinessCustomer().getCustomer()
							.getPathName());
				} else {
					map.put("sfmc", "");
				}
				map.put("yhhm", "");
				map.put("yhzh", "");
			} else if ((temp.getCooperator() != null)
					&& (temp.getCooperator().getDwmc() != null)) {
			...
			map.put("fklx",
					(String) StaticUtils.getFklxMap().get(temp.getFklx()));
			map.put("sffs",
					(String) StaticUtils.getSffsMap().get(temp.getSffs()));
			...
			map.put("wsk", getFormatMoney(temp.getBusiness().getYgs()
					- temp.getBusiness().getYjs()));
			...
			if ((temp.getBusiness() != null)
					&& (temp.getBusiness().getTravelLine() != null)) {
	...
	public String onReceiptsList() {
		if (this.account == null) {
			...
			if (StringUtils.isEmpty(this.account.getBeginDate())) {
				this.account.setBeginDate(DateUtil.afterNDay(new Date(), -100,
						"yyyy-MM-dd"));
			}
			if (StringUtils.isEmpty(this.account.getEndDate())) {
				this.account.setEndDate(DateUtil.DateToString(new Date(),
						"yyyy-MM-dd"));
			}
		...
		this.page = this.accountService
				.findOnTrustPage(this.page, this.account);
	...
	public String onPaymentList() {
		...
		this.page = this.accountService.findOnPaymentPage(this.page,
				this.account);
	...
	public String paymentInput() {
		if (this.ids != null) {
			...
			if ("1".equals(this.account.getSflx())) {
				...
				if ((this.account.getCooperator() != null)
						&& (this.account.getCooperator().getDwmc() != null)) {
					this.account
							.setDwmc(this.account.getCooperator().getDwmc());
				} else {
		...
		try {
			if ((this.account_ids != null) && (!"".equals(this.account_ids))) {
				...
				result = this.accountService
						.doAccountRefer(array, this.account);
			} else {
				result = "请勾选数据不能为空！";
			}
		} catch (Exception e) {
	...
	public List<Cooperator> getCooperatorList() {
		if (this.cooperatorList == null) {
			StringBuffer hqlSb = new StringBuffer(
					" from Cooperator u where 1=1 order by dwmc");
			Map map = new HashMap();
			this.cooperatorList = this.cooperatorService.find(hqlSb.toString(),
					map);
		}
