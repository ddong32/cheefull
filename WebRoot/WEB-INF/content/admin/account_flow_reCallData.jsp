...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
        ...
        function audit(id){
        	var url = "" ;
        	if(null == id || "" == id){
        		url = "${ctx}/admin/account_flow!audit.action?type=waitAudit" + getActionAddParam("&");
        	}else{
        		url = "${ctx}/admin/account_flow!audit.action?type=waitAudit&id=" + id + getActionAddParam("&");
        	}
       	  	var oldUrl    = document.forms[0].action;
		  	var oldTarget = document.forms[0].target;
		  	auditWindow   = getOpenWindownName()
		  	document.forms[0].action = url;
		    document.forms[0].target = auditWindow;
	...
	<body class="list con_r">
		<form id="listForm" action="${ctx}/admin/account_flow!list.action" method="post">
    	<div class="pub_inp_bg"><div class="pub_inp">
			<table class="con_table">
				<tr>
					<td align="right" width="80">提交日期：</td>
			...
			<table id="listTable" class="listTable" cellpadding="0" cellspacing="0">
				...
				<s:iterator value="page.result" var="list" status="status">
					<tr ondblclick="audit('<s:property value="#list[0]"/>');" <s:if test="#status.count%2==1">class="cor"</s:if>>
						<td class="tips"><s:property value="accoutStatMap.get(#list[1])"/></td>
						<td class="tips"><s:property value="#list[2]"/></td>
						<td class="tips"><s:property value="#list[3]"/></td>
						<td class="tips"><s:date name="#list[4]" format="yyyy-MM-dd"/></td>
						<td class="tips red tar"><s:property value="getFormatMoney(#list[5])"/></td>
						<td class="tips green tar"><s:property value="getFormatMoney(#list[6])"/></td>
						<td class="tips"><s:date name="#list[7]" format="yyyy-MM-dd"/></td>
